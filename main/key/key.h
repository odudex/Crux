 ...
 * @param fingerprint_out Output buffer for fingerprint (must be at least BIP32_KEY_FINGERPRINT_LEN bytes)
 * @return true on success, false on failure
 */
 ...
 * @param hex_out Output buffer for hex string (must be at least BIP32_KEY_FINGERPRINT_LEN*2+1 bytes)
 * @return true on success, false on failure
 */
 ...
 * @param xpub_out Output buffer for xpub string (allocated by libwally, must be freed with wally_free_string)
 * @return true on success, false on failure
 */
