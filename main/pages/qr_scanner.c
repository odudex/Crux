...
// Other variables
// static ppa_client_handle_t ppa_client_srm_handle = NULL;  //pipeline SRM
// handle
...
void camera_init(void) {
  ...
  ESP_LOGI(TAG, "Camera device opened successfully, handle: %d",
           _camera_ctlr_handle);
  ...
  // size_t detect_buf_size = ALIGN_UP_BY(hor_res * ver_res *
  // BSP_LCD_BITS_PER_PIXEL / 8, data_cache_line_size);
...
void qr_scanner_page_destroy(void) {
  ...
  if (_camera_ctlr_handle >= 0) {
    ...
    if (close_err != ESP_OK) {
      ESP_LOGE(TAG, "Failed to close video device: %s",
...
//     camera_pipeline_buffer_element *p = (camera_pipeline_buffer_element
//     *)user_data; camera_pipeline_done_element(feed_pipeline, p);
...
static void camera_video_frame_operation(uint8_t *camera_buf,
  ...
  if ((current_bits & CAMERA_EVENT_TASK_RUN) &&
    ...
    if (display_buffer && display_buffer_mutex &&
        xSemaphoreTake(display_buffer_mutex, pdMS_TO_TICKS(10)) == pdTRUE) {
    ...
    if (qr_frame_queue &&
        current_time - last_qr_analysis > 1000000) { // Increased to 1 second
      ...
      if (frame_copy) {
        ...
        qr_frame_data_t frame_data = {.frame_data = frame_copy,
...
static void rgb565_to_grayscale_downsample(
    const uint8_t *rgb565_data, uint8_t *gray_data, uint32_t src_width,
    uint32_t src_height, uint32_t dst_width, uint32_t dst_height,
    uint32_t roi_x, uint32_t roi_y, uint32_t roi_width, uint32_t roi_height) {
  const uint16_t *pixels = (const uint16_t *)rgb565_data;
  ...
  for (uint32_t dst_y = 0; dst_y < dst_height; dst_y++) {
    for (uint32_t dst_x = 0; dst_x < dst_width; dst_x++) {
      ...
      gray_data[dst_y * dst_width + dst_x] =
          (uint8_t)(0.299 * r + 0.587 * g + 0.114 * b);
    }
  }
}
...
static void qr_decode_task(void *pvParameters) {
  ...
  while (xQueueReceive(qr_frame_queue, &frame_data, portMAX_DELAY) == pdTRUE) {
    ...
    rgb565_to_grayscale_downsample(frame_data.frame_data, grayscale_buffer,
                                   ...
                                   decode_width, decode_height, roi_x, roi_y,
...
static bool qr_decoder_init(uint32_t width, uint32_t height) {
  ...
  ESP_LOGI(TAG, "Initializing QR decoder: source %dx%d -> decode %dx%d", width,
           height, decode_width, decode_height);
  ...
  if (quirc_resize(qr_decoder, decode_width, decode_height) < 0) {
    ESP_LOGE(TAG, "Failed to resize quirc decoder to %dx%d", decode_width,
             decode_height);
