...
ui_menu_t *ui_menu_create(lv_obj_t *parent, const char *title) {
  ...
  // lv_obj_set_style_pad_bottom(menu->title_label, 20, 0);  // Add spacing
  // below title
  ...
  lv_obj_set_size(menu->list, LV_PCT(100),
                  LV_PCT(100)); // Use full width and height
  lv_obj_set_flex_flow(menu->list, LV_FLEX_FLOW_COLUMN);
  lv_obj_set_flex_align(menu->list, LV_FLEX_ALIGN_START,
                        LV_FLEX_ALIGN_SPACE_EVENLY,
                        LV_FLEX_ALIGN_CENTER); // Stretch to fill
...
bool ui_menu_add_entry(ui_menu_t *menu, const char *name,
  ...
  // Configure entry
  strncpy(menu->config.entries[index].name, name,
          UI_MENU_ENTRY_NAME_MAX_LEN - 1);
  ...
  lv_obj_set_flex_grow(menu->buttons[index],
                       1); // Make buttons grow to fill available space
  lv_obj_add_event_cb(menu->buttons[index], menu_button_event_cb,
  ...
  lv_obj_set_style_pad_ver(label, 15,
                           0); // Add vertical padding for better button height
  lv_obj_center(label);
...
bool ui_menu_execute_selected(ui_menu_t *menu) {
  ...
  if (menu->config.entries[index].callback) {
    ESP_LOGI(TAG, "Executing callback for entry '%s'",
             menu->config.entries[index].name);
    menu->config.entries[index].callback();
    return true;
  }

  ESP_LOGW(TAG, "No callback set for entry '%s'",
           menu->config.entries[index].name);
  return false;
}
